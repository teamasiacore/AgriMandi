...
17. **Buyer Verification Sync & Bidding Unlock Resolution (P0)**:
    - **Issue**: After SuperAdmin approved a buyer, `BuyerPortal.jsx` continued to show the warning banner and prevented bidding with the alert: *"Your account is under administrative review by ASIACore..."*.
    - **Root Cause**: `BuyerPortal.jsx` only read stale `localStorage.getItem('agri_user')` and never fetched the fresh profile from the database. Furthermore, `authRoutes.js` failed to merge `buyerProfile` into `res.user` during login and lacked a `GET /auth/buyer/profile/:identifier` endpoint.
    - **Fix Applied**: Added `GET /auth/buyer/profile/:identifier` in `authRoutes.js`, added `api.getBuyerProfile` in `api.js`, updated `BuyerPortal.jsx` to fetch the authoritative profile on mount and sync `localStorage`, fixed `is_verified` boolean checks, and updated `handleOpenBidModal` to immediately unlock live counter-bidding upon verification.
